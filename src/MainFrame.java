...
public class MainFrame implements ActionListener
{
	...
	private void createPanelM()
	{
		...
		for(int i = 0; i < 10; i++)
		{
			for(int j = 0; j < 10; j++)
			{
				...
				buttonM[j][i].setBackground(Color.RED);
	...
	private void createPanelE()
	{
		...
		for(int i = 0; i < 10; i++)
		{
			for(int j = 0; j < 10; j++)
			{
				...
				buttonE[j][i].setText(".");
				buttonE[j][i].setBackground(Color.red);
	...
	private void changeButtonE(int x, int y)
	{
		...
		if(txt.equals("X")){
			buttonE[x][y].setBackground(new Color(0,255,0));
			buttonE[x][y].setOpaque(true);}
		else{
			buttonE[x][y].setBackground(new Color(204,0,0));
			buttonE[x][y].setOpaque(true);}
	}
	private void changeButtonM()
	{
		for(int i = 0; i < 10; i++)
		{
			for(int j = 0; j < 10; j++)
			{
				...
				if(txt.equals("X")){
					buttonM[j][i].setBackground(new Color(255,0,0));
					buttonM[j][i].setOpaque(true);}
				else if(txt.equals("+")){
					buttonM[j][i].setBackground(new Color(234,255,0));
					buttonM[j][i].setOpaque(true);}
			}
		}
	}
